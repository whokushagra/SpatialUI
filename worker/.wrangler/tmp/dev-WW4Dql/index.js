...
// .wrangler/tmp/bundle-GmW6xL/strip-cf-connecting-ip-header.js
...
var SessionRoom = class {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/claim" && request.method === "POST") {
      ...
      const ttl = claimed ? 10 * 60 * 1e3 : 5 * 60 * 1e3;
      if (idleMs > ttl) {
        this.sessionId = null;
        this.plainPin = null;
        this.claimToken = null;
      ...
      if (pinHash !== expected) {
        ...
        if (this.pinAttemptsRemaining <= 0) {
          if (this.desktopWs) {
            try {
              this.desktopWs.send(JSON.stringify({ kind: "pairing-canceled", reason: "too-many-attempts" }));
            } catch {
            }
          }
          this.sessionId = null;
      ...
      if (this.desktopWs) {
        try {
          this.desktopWs.send(JSON.stringify({ kind: "phone-claimed" }));
        } catch {
        }
      }
...
__name(SessionRoom, "SessionRoom");
...
__name(randHex2, "randHex");
function randPin() {
  const n = crypto.getRandomValues(new Uint32Array(1))[0] % 1e4;
  return n.toString().padStart(4, "0");
}
__name(randPin, "randPin");
async function handleSignalNew(request, env2) {
  if (request.method !== "POST")
    return new Response("method not allowed", { status: 405 });
  const sessionId = randHex2(16);
  const pin = randPin();
  const id = env2.SESSION_ROOM.idFromName(sessionId);
...
__name(handleSignalNew, "handleSignalNew");
async function handleSignalClaim(request, env2) {
  if (request.method !== "POST")
    return new Response("method not allowed", { status: 405 });
  const { sessionId, pinHash } = await request.json();
  if (!sessionId || !pinHash)
...
__name(handleSignalClaim, "handleSignalClaim");
async function handleSignalWs(request, env2) {
  const url = new URL(request.url);
  const role = url.searchParams.get("role");
...
__name(handleSignalWs, "handleSignalWs");
var CORS_HEADERS = {
...
var src_default = {
  async fetch(request, env2) {
    ...
    const url = new URL(request.url);
    let res;
    if (url.pathname === "/api/signal/new")
      res = await handleSignalNew(request, env2);
    else if (url.pathname === "/api/signal/claim")
      res = await handleSignalClaim(request, env2);
    else if (url.pathname === "/api/signal/ws")
      return handleSignalWs(request, env2);
    else
...
// .wrangler/tmp/bundle-GmW6xL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-GmW6xL/middleware-loader.entry.ts
